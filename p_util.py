...
from p_transitions import minibatch_parse
import numpy as np
...
class Parser(object):
    ...
    def __init__(self, dataset):
        ...
        tok2id = {l_pr + l: i for (i, l) in enumerate(deprel)}
        tok2id[l_pr + NULL] = self.L_NULL = len(tok2id)
        ...
        tok2id.update(build_dict([p_pr + w for ex in dataset for w in ex['pos']],
                                 offset=len(tok2id)))
        tok2id[p_pr + UNK] = self.P_UNK = len(tok2id)
        tok2id[p_pr + NULL] = self.P_NULL = len(tok2id)
        tok2id[p_pr + ROOT] = self.P_ROOT = len(tok2id)
    ...
    def vectorize(self, examples):
        ...
        for ex in examples:
            ...
            pos = [self.P_ROOT] + [self.tok2id[p_pr + w] if p_pr + w in self.tok2id
                                   else self.P_UNK for w in ex['pos']]
            head = [-1] + ex['head']
            label = [-1] + [self.tok2id[l_pr + w] if l_pr + w in self.tok2id
                            else -1 for w in ex['label']]
            vec_examples.append({'word': word, 'pos': pos,
                                 'head': head, 'label': label})
        return vec_examples

    def fet_ext(self, stack, buf, arcs, ex):
        if stack[0] == "P_ROOT":
            stack[0] = 0
    ...
    def create_instances(self, examples):
        ...
        for id, ex in enumerate(examples):
            ...
            for i in range(n_words * 2):
                ...
                instances.append((self.fet_ext(stack, buf, arcs, ex),
                                  legal_labels, gold_t))
    ...
    def parse(self, dataset, eval_batch_size=5000):
        ...
        for i, ex in enumerate(dataset):
            ...
            for pred_h, gold_h, gold_l, pos in \
                    ...
                    assert self.id2tok[pos].startswith(p_pr)
                    pos_str = self.id2tok[pos][len(p_pr):]
...
class ModelWrapper(object):
    ...
    def predict(self, partial_parses):
        mb_x = [(self.parser.fet_ext(p.stack, p.buffer, p.dependencies, self.dataset[self.sentence_id_to_idx[id(p.sentence)]]))
